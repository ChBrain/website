...
describe("CVI - design contract", () => {
  ...
  describe("§02 the set - monogram + icon families", () => {
    it("§02·ii renders the 5-variant kh monogram family", () => {
      ...
      expect(s, "missing section#set-monogram").not.toBeNull();
      expect([...s!.querySelectorAll(".cvi-card")].length).toBe(5);
    });

    it("§02·iii renders the 4-tile app-icon family", () => {
      const dom = new JSDOM(cvi!.html);
      const s = dom.window.document.querySelector("section#set-icon");
      expect(s, "missing section#set-icon").not.toBeNull();
      expect([...s!.querySelectorAll(".cvi-card")].length).toBe(4);
    });
  });
