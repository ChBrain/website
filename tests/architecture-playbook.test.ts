...
// installed engines are gender and language; this set is the expected set when
// rendered. The detection keeps the gate green on the pre-render build and
// locks the placement + WIRES chapters once the render lands.
const enginesRendered = playbookPage
  ? new JSDOM(playbookPage.html).window.document.querySelectorAll("article.pb-spread--engine")
      .length > 0
  : false;
const ENGINE_SLUGS = enginesRendered ? ["gender", "language"] : [];
